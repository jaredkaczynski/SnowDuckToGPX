...
def main():
    ...
    # PARSE!
    # output = (parser.safe_call(parser.match_handler("message"), stdin.buffer, root_type) + "\n")
    with open('test.txt', 'rb') as fh:
        output = parser.parse_message(fh, root_type)
        with open('buf1.txt', 'w') as out:
            out.write(output)
        print("converting to JSON")
        # run(output, write_json=True, print_json=False)
